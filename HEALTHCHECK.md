...
| **Duplicate Deribit clients** (trading vs public data) | Maintenance burden; any bug fix or API change must be applied twice. | Low-to-moderate risk; the clients serve different purposes (auth vs no-auth), so they can share base code without full merge. | **FIXED** – Now uses `src/deribit/base_client.py` |
...
| **Deribit clients** | `src/deribit_client.py:DeribitClient` vs `src/backtest/deribit_client.py:DeribitPublicClient` | Add public endpoints to main client, or create shared base class. | **FIXED** – Uses `src/deribit/base_client.py` |
