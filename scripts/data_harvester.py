...
INTERVAL_SECONDS = int(os.getenv("HARVESTER_INTERVAL_SECONDS", "900"))
...
def fetch_option_chain(currency: str) -> list[dict[str, Any]]:
    ...
    try:
        ...
        if "result" in data:
            ...
            logger.warning(
                f"No 'result' key in response for {currency}: {data.get('error', 'unknown error')}"
            )
            return []
...
def run_harvester() -> None:
    ...
    logger.info(
        f"Polling interval: {INTERVAL_SECONDS} seconds ({INTERVAL_SECONDS / 60:.1f} min)"
    )
