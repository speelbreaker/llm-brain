...
def map_real_scraper_columns(
    df: pd.DataFrame, underlying: str, target_date: str
) -> pd.DataFrame:
    """
    ...
    if "instrument_name" in df.columns:
        ...
        for col in expiry_cols:
            if col in df.columns:
                try:
                    ...
                    result["expiry_timestamp"] = exp_dt_series.apply(
        ...
        if "instrument_name" not in result.columns and all(
            ...
            def make_name(row):
                try:
                    exp_dt = datetime.strptime(str(row["expiry"]), "%Y-%m-%d").replace(
                        tzinfo=timezone.utc
                    )
...
def print_summary(df: pd.DataFrame, underlying: str, target_date: str) -> None:
    ...
    if "mark_price" in df.columns:
        ...
        if len(valid_marks) > 0:
            print(
                f"Mark price range:     {valid_marks.min():.6f} - {valid_marks.max():.6f}"
            )
    ...
    if "underlying_price" in df.columns:
        ...
        if len(valid_spots) > 0:
            print(
                f"Spot price range:     ${valid_spots.min():,.2f} - ${valid_spots.max():,.2f}"
            )
...
def main():
    ...
    print("Importing Real Scraper data")
