...
def main() -> None:
    ...
    print("Building calibration config...")
    ...
    if result.bands:
        ...
        for band in result.bands:
            ...
            band_vmae = (
                f"{band.vega_weighted_mae_pct:.2f}%"
                if band.vega_weighted_mae_pct
                else "-"
            )
            print(
                f"  {band.name}: mult={band_mult}, vMAE={band_vmae}, count={band.count}"
            )
