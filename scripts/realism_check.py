...
def load_harvested_sensors(
    ...
    quality_summary = (
        aggregate_quality_reports(quality_reports) if validate_quality else None
    )
    ...
    daily = (
        combined.groupby(combined["harvest_time"].dt.date)
        .agg(
            {
    ...
    daily["rv_7d"] = (
        daily["log_ret"].rolling(7, min_periods=5).std() * np.sqrt(365) * 100
    )
    daily["rv_30d"] = (
        daily["log_ret"].rolling(30, min_periods=20).std() * np.sqrt(365) * 100
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Compare synthetic vs real distributions"
    )

    parser.add_argument(
        "--underlying", "-u", type=str, default="BTC", help="Underlying asset"
    )
    parser.add_argument(
        "--data-root",
        type=str,
    ...
    parser.add_argument(
        "--days", type=int, default=90, help="Lookback days for real data"
    )
    parser.add_argument(
        "--synthetic-days", type=int, default=180, help="Number of days to simulate"
    )
    parser.add_argument(
        "--seed", type=int, default=42, help="Random seed for synthetic generation"
    )
    parser.add_argument(
    ...
    if quality_summary:
        if quality_summary.status == DataQualityStatus.FAILED:
            ...
            print("  WARNING: Data quality is degraded.")
            print(f"    {'; '.join(quality_summary.issues)}")
            data_quality_warning = True
    ...
    for sensor, stats in moment_diffs.items():
        print(
            f"  {sensor:15s}: real_mean={stats['real_mean']:8.3f}, synth_mean={stats['synth_mean']:8.3f}, KS={stats['ks_distance']:.3f}"
        )
    ...
    if data_quality_warning:
        ...
        print(
            f"NOTE: Data quality issues detected. Realism score adjusted from {realism_score:.2f} to {adjusted_score:.2f}"
        )
