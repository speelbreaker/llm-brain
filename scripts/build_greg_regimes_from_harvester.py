...
def compute_sensors_from_ohlc(
    ...
    range_size = (
        df["high"].rolling(7, min_periods=5).max()
        - df["low"].rolling(7, min_periods=5).min()
    )
    df["chop_factor_7d"] = (
...
def main():
    ...
    if len(spot_df) < min_required:
        print(
            f"WARNING: Only {len(spot_df)} data points available, minimum {min_required} recommended."
        )
        if len(spot_df) < args.n_clusters * 2:
            print(
                f"ERROR: Not enough data for {args.n_clusters} clusters. Reduce --n-clusters or increase --lookback-days."
            )
            sys.exit(1)
