...
def generate_text_report(
    ...
    if by_action:
        for action, data in sorted(by_action.items()):
            ...
            rate = (exec_ / sugg * 100) if sugg > 0 else 0
            lines.append(
                f"  {action}: {sugg} suggested, {exec_} executed ({rate:.0f}%)"
            )
    else:
    ...
    lines.append(
        f"  Avg PnL when Followed: {followed_avg * 100:.1f}%"
        if followed_avg
        else "  Avg PnL when Followed: N/A"
    )
    lines.append(
        f"  Avg PnL when Ignored: {ignored_avg * 100:.1f}%"
        if ignored_avg
        else "  Avg PnL when Ignored: N/A"
    )
    lines.append("")
    ...
    if followed_avg is not None and ignored_avg is not None:
        ...
        lines.append(
            f"  Impact: Following suggestions was {abs(diff) * 100:.1f}% {direction} on average."
        )
    lines.append("")
    ...
    for entry in history[:20]:
        ...
        lines.append(
            f"  [{ts}] {underlying} {action:12} {executed} ({mode}) PnL={pnl_str}"
        )
