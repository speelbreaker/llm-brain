...
from src.calibration_extended import (
    run_calibration_extended,
    build_vol_surface_from_calibration,
)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--min-dte", type=float, default=3.0, help="Minimum DTE for calibration"
    )
    parser.add_argument(
        "--max-dte", type=float, default=30.0, help="Maximum DTE for calibration"
    )
    parser.add_argument(
        "--iv-multiplier", type=float, default=1.0, help="Current IV multiplier"
    )
    parser.add_argument("--rv-window", type=int, default=7, help="RV window in days")
    parser.add_argument(
        "--max-samples", type=int, default=80, help="Max samples to process"
    )
    ...
    parser.add_argument(
        "--fit-skew", action="store_true", help="Fit skew anchor ratios"
    )
    parser.add_argument(
        "--bands",
        type=str,
    ...
    parser.add_argument(
        "--config", type=str, default=None, help="Path to calibration config JSON file"
    )
...
def run_with_policy(args: argparse.Namespace) -> int:
    ...
    source: Literal["live", "harvested"] = (
        "live" if args.source == "live" else "harvested"
    )
    ...
    print(
        f"  Policy: min_delta={policy.min_delta_global}, min_samples={policy.min_sample_size}, smoothing={policy.smoothing_window_days}d",
        file=sys.stderr,
    )
    if args.force:
        print("  FORCE mode: will apply regardless of thresholds", file=sys.stderr)
    ...
    print(
        f"  Recommended IV multiplier: {record.recommended_iv_multiplier:.4f}",
        file=sys.stderr,
    )
    print(
        f"  Smoothed IV multiplier: {record.smoothed_global_multiplier:.4f}",
        file=sys.stderr,
    )

    if record.recommended_band_multipliers:
        print("  Band multipliers:", file=sys.stderr)
        for band in record.recommended_band_multipliers:
            ...
            print(
                f"    {band.name}: {band.iv_multiplier:.4f}{smoothed_str}",
                file=sys.stderr,
            )
...
def run_legacy_mode(args: argparse.Namespace) -> int:
    ...
    print(
        f"Running calibration for {config.underlying} (legacy mode)...", file=sys.stderr
    )
    ...
    print(
        f"  RV (annualized): {result.rv_annualized:.2%}"
        if result.rv_annualized
        else "",
        file=sys.stderr,
    )
    print(
        f"  Recommended IV multiplier: {result.recommended_iv_multiplier}"
        if result.recommended_iv_multiplier
        else "",
        file=sys.stderr,
    )
    ...
    if result.vol_surface_diff:
        ...
        print(
            f"  IV multiplier delta: {result.vol_surface_diff.iv_multiplier_delta:+.4f}",
            file=sys.stderr,
        )
