...
- **AI Steward (Project Brain)**: A project planning and QA helper (`src/system_steward.py`) that reads meta-docs (ROADMAP, BACKLOG, UI gaps, HEALTHCHECK, replit.md) and uses LLM to summarize project state and suggest next tasks. Exposes `POST /api/steward/run` and `GET /api/steward/report` endpoints. The System Health tab includes an interactive "AI Steward" panel with run button, summary display, top suggestions list, and a copyable builder prompt for AI assistance. Never interacts with Deribit or trading.
