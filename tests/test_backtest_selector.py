...
def test_selector_name_in_config_dict():
    ...
    for selector_name, description in test_cases:
        ...
        assert "selector_name" in config_dict, (
            f"selector_name missing from config for {description}"
        )
        assert config_dict["selector_name"] == selector_name, (
            f"Wrong selector_name for {description}"
        )
...
def test_backtest_request_model_has_selector():
    ...
    req = BacktestStartRequest(
        start="2025-12-07", end="2025-12-08", selector_name="greg_vrp_harvester"
    )
