...
class TestLiveChainMarkPreservation:
    ...
    def test_multiplier_not_1_recalculates_marks(self):
        ...
        expected_price = bs_call_price(
            spot, 70000, t_years, expected_scaled_iv, cfg.risk_free_rate
        )
...
class TestLiveChainDebugSamples:
    ...
    def test_debug_sample_collects_comparison_data(self):
        ...
        expected_bs_price = bs_call_price(
            spot, 70000, 7.0 / 365.0, original_iv, cfg.risk_free_rate
        )
        assert abs(sample.engine_price - expected_bs_price) < 0.01

        expected_diff_pct = (
            abs(expected_bs_price - original_mark_price) / original_mark_price * 100.0
        )
...
class TestGregVRPIVSensitivity:
    ...
    def test_greg_vrp_is_sensitive_to_iv_multiplier(self):
        ...
        assert (
            metrics_high["net_profit_pct"]
            >= metrics_low["net_profit_pct"] - profit_tolerance
        ), (
