...
def test_loop_invariants_lint_only(tmp_path, monkeypatch):
    ...
    with TestClient(supervisor_app, raise_server_exceptions=False) as client:
        ...
        assert "Check Results" in comment_bodies[0]
        assert "Arbiter" in comment_bodies[0]
        assert "Auto-fix" in comment_bodies[-1]
        ...
        assert settings.github_token not in comment_bodies[0]


def test_lint_only_dry_run_with_codex_unavailable(tmp_path, monkeypatch):
    settings = SupervisorSettings()
    settings.enabled = True
    settings.github_webhook_secret = "test_secret"
    settings.github_token = "ghp_testsecret1234567890"
    ...
    settings.codex_bin = "/nonexistent-codex-drill-lint"
    assert not settings.is_codex_available()
    settings.telegram_enabled = False
    settings.base_jobs_dir = str(tmp_path)
    ...
    github_client = SimpleNamespace(
        ...
        get_pr_info=AsyncMock(return_value={"labels": []}),
        post_pr_comment=AsyncMock(side_effect=lambda repo, pr_number, body: comment_bodies.append(body) or {"id": 1}),
        update_pr_comment=AsyncMock(side_effect=lambda repo, comment_id, body: comment_bodies.append(body) or {"id": comment_id}),
    )
    ...
    with TestClient(supervisor_app, raise_server_exceptions=False) as client:
        signature = make_signature(json.dumps(payload).encode(), settings.github_webhook_secret)
        response = client.post(
            "/github/webhook",
        ...
        assert comment_bodies
        assert "DRY RUN" in comment_bodies[-1]


def test_non_lint_failure_without_codex_fails_fast(tmp_path, monkeypatch):
    settings = SupervisorSettings()
    settings.enabled = True
    settings.github_webhook_secret = "test_secret"
    settings.github_token = "ghp_testsecret1234567890"
    ...
    settings.autofix_dry_run = True
    settings.enable_codex = True
    settings.codex_bin = "/nonexistent-codex-drill-test"
    assert not settings.is_codex_available()
    settings.telegram_enabled = False
    settings.base_jobs_dir = str(tmp_path)
    ...
    class NonLintRunner:
        async def run_checks(self, _workspace_path: str, head_sha: str) -> VerificationReport:
            return VerificationReport(
                ...
                failing_tests=["tests/test_example.py::test_example"],
            )

    fake_runner = NonLintRunner()

    async def fake_apply_fix_plan(*_args, **_kwargs):
        return FixResult(False, "unsupported", [], ["No deterministic fixer"])

    def fake_propose_fix_plan(*_args, **_kwargs):
        return FixPlan(category="unknown", objectives=[], approach="", estimated_risk="low")
    ...
    class RaisingCodexFixer:
        async def apply_fix(self, *_args, **_kwargs):
            raise AssertionError("Codex should not be invoked when unavailable")
    ...
    monkeypatch.setattr(app_module, "WorkspaceManager", lambda _settings: fake_workspace)
    monkeypatch.setattr(app_module, "VerificationRunner", lambda _settings: fake_runner)
    monkeypatch.setattr(app_module, "CodexFixer", lambda _settings: RaisingCodexFixer())
    monkeypatch.setattr(app_module, "TelegramNotifier", FakeNotifier)
    monkeypatch.setattr(app_module, "apply_fix_plan", fake_apply_fix_plan)
    monkeypatch.setattr(app_module, "propose_fix_plan", fake_propose_fix_plan)
    monkeypatch.setattr(app_module, "review_fix_plan", fake_review_fix_plan)
    ...
    with TestClient(supervisor_app, raise_server_exceptions=False) as client:
        ...
        assert job.reason_code == "CODEX_UNAVAILABLE"
        assert "Codex auto-fixes require" in job.final_message
        assert comment_bodies
