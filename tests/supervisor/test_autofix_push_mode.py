...
class FakeWorkspaceManager:
    ...
    async def get_diff_stats(self, workspace_path):
        return DiffStats(files_changed=1, lines_added=0, lines_removed=1, total_loc_changed=1)
...
class FakeDebateSystem:
    ...
    async def run_debate(self, **kwargs):
        return ArbiterDecision(
            ...
            fix_objectives=["fix ruff"],
            risk_level="low",
        )


@pytest.mark.asyncio
async def test_autofix_push_mode_commits_and_pushes(monkeypatch, tmp_path):
    ...
    commit_and_push_mock = AsyncMock(return_value="beefcafe")
    monkeypatch.setattr(app_module.CodexFixer, "apply_fix", apply_fix_mock)
    monkeypatch.setattr(FakeWorkspaceManager, "commit_and_push", commit_and_push_mock)
    ...
    class NoopNotifier:
        ...
        async def notify_fix_pushed(self, *args, **kwargs):
            return None
    ...
    assert "Fixed and pushed" in job.final_message
    assert len(job.fix_attempts) == 1
    assert job.fix_attempts[0].committed is True
    assert job.fix_attempts[0].commit_sha == "beefcafe"

    apply_fix_mock.assert_awaited()
    commit_and_push_mock.assert_awaited()
