...
from src.supervisor.store import JobStore, PRApprovalState
...
class TestDebugEndpointGating:
    ...
    def test_debug_token_required_when_configured(self):
        ...
        with pytest.raises(HTTPException) as exc:
            asyncio.get_event_loop().run_until_complete(
                simulate_pr_event_handler(mock_request, body, x_debug_token="wrong-token")
            )
        assert exc.value.status_code == 401
    ...
    def test_concurrent_writes_safe(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            def writer(pr_num):
                try:
                    for i in range(5):
                        store.set_pr_approval("owner/repo", pr_num, True, user_id=pr_num * 100 + i)
                        time.sleep(0.01)
...
class TestRetryHelper:
    ...
    def test_retry_imports_cleanly(self):
        ...
        from src.supervisor.retry import with_retry, get_retry_client, RETRYABLE_STATUS_CODES
