...
from src.supervisor.commands import SupervisorCommands, CommandResult
...
class MockStore:
    ...
    def set_pr_approval(self, repo: str, pr_number: int, approved: bool, user_id=None) -> PRApprovalState:
        key = f"{repo}:{pr_number}"
    ...
    def set_pr_paused(self, repo: str, pr_number: int, paused: bool, user_id=None) -> PRApprovalState:
        key = f"{repo}:{pr_number}"
        state = PRApprovalState(
            repo=repo,
            pr_number=pr_number,
