...
class TestAggregateQualityReports:
    ...
    def test_all_ok_reports_returns_ok(self):
        reports = [
            SnapshotQualityReport(
                filename="a.parquet", total_rows=100, non_null_core_fraction=0.99
            ),
            SnapshotQualityReport(
                filename="b.parquet", total_rows=100, non_null_core_fraction=0.98
            ),
        ]
    ...
    def test_schema_failures_return_failed(self):
        reports = [
            SnapshotQualityReport(
                filename="a.parquet", schema_issues=["Missing column"]
            ),
            SnapshotQualityReport(
                filename="b.parquet", total_rows=100, non_null_core_fraction=0.99
            ),
        ]
        summary = aggregate_quality_reports(reports)
    ...
    def test_total_rows_aggregated(self):
        reports = [
            ...
            SnapshotQualityReport(
                filename="b.parquet", total_rows=200, non_null_core_fraction=0.95
            ),
        ]
        summary = aggregate_quality_reports(reports)
    ...
    def test_weighted_fraction_calculated(self):
        reports = [
            SnapshotQualityReport(
                filename="a.parquet", total_rows=100, non_null_core_fraction=1.0
            ),
            SnapshotQualityReport(
                filename="b.parquet", total_rows=100, non_null_core_fraction=0.8
            ),
        ]
        summary = aggregate_quality_reports(reports)
