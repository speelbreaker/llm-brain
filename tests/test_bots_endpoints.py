...
class TestBotsStrategies:
    ...
    def test_gregbot_present(self):
        ...
        assert data.get("ok") is True
        greg_strategies = [
            s for s in data["strategies"] if s.get("expert_id") == "greg_mandolini"
        ]
...
class TestBotsTypes:
    ...
    def test_strategy_criterion_model(self):
        ...
        criterion = StrategyCriterion(
            metric="vrp_30d", value=8.5, min=5.0, max=None, ok=True, note="ok"
        )
    ...
    def test_strategy_evaluation_model(self):
        ...
        evaluation = StrategyEvaluation(
            ...
            criteria=[StrategyCriterion(metric="vrp_30d", value=8.5, ok=True)],
        )
...
class TestETHSensors:
    ...
    def test_eth_ohlc_sensors_computed(self):
        ...
        assert eth_sensors.get("price_vs_ma200") is not None, (
            "ETH price_vs_ma200 should be computed"
        )
...
class TestSkew25d:
    ...
    def test_skew_25d_field_exists_for_btc(self):
        ...
        data = response.json()
        assert data.get("ok") is True
        assert "skew_25d" in data["sensors"].get("BTC", {}), (
            "BTC should have skew_25d field"
        )
    ...
    def test_skew_25d_field_exists_for_eth(self):
        ...
        data = response.json()
        assert data.get("ok") is True
        assert "skew_25d" in data["sensors"].get("ETH", {}), (
            "ETH should have skew_25d field"
        )
    ...
    def test_strategies_using_skew_have_criterion(self):
        ...
        for strat in data["strategies"]:
            if strat["strategy_key"] in skew_strategies:
                ...
                assert "skew_25d" in criteria_metrics, (
                    f"{strat['strategy_key']} should have skew_25d criterion"
                )
    ...
    def test_compute_skew_25d_returns_numeric_or_none(self):
        ...
        if not df.empty:
            ...
            assert skew is None or isinstance(skew, float), (
                "skew_25d should be float or None"
            )
