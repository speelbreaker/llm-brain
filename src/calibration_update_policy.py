...
def should_apply_update(
    ...
    if sample_size < policy.min_sample_size:
        return UpdateDecision(
            ...
            details={
                "sample_size": sample_size,
                "min_required": policy.min_sample_size,
            },
        )
    ...
    if band_passes:
        passing_bands = [
            n for n, d in band_deltas.items() if d >= policy.min_delta_band
        ]
...
def get_current_applied_multipliers(
    underlying: str = "BTC",
...
def run_calibration_with_policy(
    ...
    **config_kwargs,
    ...
    vega_sum = 0.0
    if result.global_metrics and hasattr(
        result.global_metrics, "vega_weighted_mae_pct"
    ):
