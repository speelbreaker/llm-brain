...
def _compute_worst_severity(result: dict) -> Optional[HealthSeverity]:
    ...
    for check in result.get("results", []):
        if check.get("status") in ("fail", "warn"):
            ...
            if worst is None or severity_order.get(sev, 0) > severity_order.get(
                worst, 0
            ):
                worst = sev
...
def get_health_status_for_api() -> dict:
    ...
    return {
        ...
        "worst_severity": cached.worst_severity.value
        if cached.worst_severity
        else None,
...
def _validate_basic_config(cfg: Settings) -> list[str]:
    ...
    if not (0 < cfg.max_margin_used_pct <= 100):
        issues.append(
            f"max_margin_used_pct must be in (0, 100], got {cfg.max_margin_used_pct}"
        )
...
def _validate_risk_settings(cfg: Settings) -> tuple[list[str], list[str]]:
    ...
    if cfg.daily_drawdown_limit_pct == 0.0:
        ...
        failures.append(
            f"daily_drawdown_limit_pct must be >= 0, got {cfg.daily_drawdown_limit_pct}"
        )

    if cfg.max_expiry_exposure <= 0:
        failures.append(
            f"max_expiry_exposure must be > 0, got {cfg.max_expiry_exposure}"
        )

    if (
        hasattr(cfg, "research_max_expiry_exposure")
        and cfg.research_max_expiry_exposure <= 0
    ):
        warnings.append(
            f"research_max_expiry_exposure is <= 0: {cfg.research_max_expiry_exposure}"
        )
...
def _validate_llm_settings(cfg: Settings) -> tuple[list[str], list[str]]:
    ...
    if cfg.llm_timeout_seconds <= 0:
        warnings.append(
            f"llm_timeout_seconds should be > 0, got {cfg.llm_timeout_seconds}"
        )

    if cfg.llm_max_decision_tokens <= 0:
        warnings.append(
            f"llm_max_decision_tokens should be > 0, got {cfg.llm_max_decision_tokens}"
        )
    ...
    try:
        ...
        if basic_issues:
            return HealthCheckResult(
                name="config", status=CheckStatus.FAIL, detail="; ".join(basic_issues)
            )

        mode = (
            "training"
            if cfg.training_mode
            else ("research" if cfg.is_research else "live")
        )
        detail = f"mode={mode}, env={cfg.deribit_env}, loop_interval={cfg.loop_interval_sec}s"
        return HealthCheckResult(name="config", status=CheckStatus.OK, detail=detail)
...
def check_risk_config(cfg: Settings) -> HealthCheckResult:
    ...
    try:
        ...
        if failures:
            return HealthCheckResult(
                name="risk_config", status=CheckStatus.FAIL, detail="; ".join(failures)
            )

        if warnings:
            return HealthCheckResult(
                name="risk_config", status=CheckStatus.WARN, detail="; ".join(warnings)
            )
...
def check_llm_config(cfg: Settings) -> HealthCheckResult:
    ...
    try:
        ...
        if failures:
            return HealthCheckResult(
                name="llm_config", status=CheckStatus.FAIL, detail="; ".join(failures)
            )

        if warnings:
            return HealthCheckResult(
                name="llm_config", status=CheckStatus.WARN, detail="; ".join(warnings)
            )
    ...
    try:
        ...
        if "credentials" in error_msg or "authentication" in error_msg:
            return HealthCheckResult(
                name="state_builder",
                status=CheckStatus.WARN,
                detail="state built with partial portfolio (no private credentials)",
            )
        return HealthCheckResult(
            name="state_builder",
            status=CheckStatus.FAIL,
...
def check_iv_sanity() -> HealthCheckResult:
    ...
    try:
        ...
        if status_str == "ok":
            return HealthCheckResult(
                name="iv_sanity", status=CheckStatus.OK, detail=summary
            )
        elif status_str == "degraded":
            return HealthCheckResult(
                name="iv_sanity", status=CheckStatus.WARN, detail=summary
            )
        else:
            return HealthCheckResult(
                name="iv_sanity", status=CheckStatus.FAIL, detail=summary
            )
    except Exception as e:
        return HealthCheckResult(
            name="iv_sanity",
