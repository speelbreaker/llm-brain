...
class DataQualitySummary:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "overall_non_null_core_fraction": round(
                self.overall_non_null_core_fraction, 4
            ),
            "total_rows": self.total_rows,
            "status": self.status.value,
            "issues": self.issues,
...
def _check_dtype_compatibility(actual_dtype, expected_type: str) -> bool:
    ...
    if expected_type == "datetime64":
        ...
        return np.issubdtype(actual_dtype, np.integer) or np.issubdtype(
            actual_dtype, np.floating
        )
    elif expected_type == "float64":
        return np.issubdtype(actual_dtype, np.floating) or np.issubdtype(
            actual_dtype, np.integer
        )
...
def aggregate_quality_reports(
    ...
    if total_rows > 0:
        weighted_fraction = (
            sum(r.non_null_core_fraction * r.total_rows for r in reports) / total_rows
        )
    ...
    if low_quality_fraction > degraded_threshold:
        ...
        summary.overall_non_null_core_fraction
        < QUALITY_THRESHOLDS["core_field_fraction"]
    ):
        summary.status = DataQualityStatus.DEGRADED
        summary.issues.append(
