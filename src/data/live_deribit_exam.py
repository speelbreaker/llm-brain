...
def compute_summary(
    ...
    summary = {
        ...
        "dte_min": float(min_dte)
        if min_dte is not None and not pd.isna(min_dte)
        else None,
        "dte_max": float(max_dte)
        if max_dte is not None and not pd.isna(max_dte)
        else None,
        "columns": columns,
...
def build_live_deribit_exam_dataset(
    ...
    if write_files:
        ...
        summary_filename = (
            f"{underlying}_{start_str}_{end_str}_live_deribit_summary.json"
        )
