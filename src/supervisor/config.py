...
class SupervisorSettings(BaseSettings):
    ...
    max_fix_attempts: int = Field(default=3, alias="SUPERVISOR_MAX_FIX_ATTEMPTS")
    max_total_runtime_seconds: int = Field(default=1800, alias="SUPERVISOR_MAX_TOTAL_RUNTIME_SECONDS")
    fix_backoff_base_seconds: float = Field(default=1.0, alias="SUPERVISOR_FIX_BACKOFF_BASE_SECONDS")
    fix_backoff_factor: float = Field(default=2.0, alias="SUPERVISOR_FIX_BACKOFF_FACTOR")
    fix_backoff_max_seconds: float = Field(default=8.0, alias="SUPERVISOR_FIX_BACKOFF_MAX_SECONDS")
