...
def check_autofix_policy(
    ...
    if not settings.enable_codex:
        return AutofixDecision(
            allowed=False, reason="Codex is disabled (SUPERVISOR_ENABLE_CODEX=0)"
        )

    approval = store.get_pr_approval(repo, pr_number)

    if approval.paused:
        return AutofixDecision(allowed=False, reason="PR is paused")
    ...
    if policy == "label":
        ...
        return AutofixDecision(
            allowed=False, reason=f"Invalid autofix policy: {policy}"
        )

    if not policy_satisfied:
        return AutofixDecision(allowed=False, reason=policy_reason)
    ...
    return AutofixDecision(allowed=True, reason="Autofix policy satisfied")
