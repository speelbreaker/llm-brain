...
class TelegramStatusCard:
    ...
    def _build_card_text(self, use_html: bool = True) -> str:
        ...
        if self.arbiter_verdict:
            lines.append("")
            verdict_text = (
                escape_html(self.arbiter_verdict) if use_html else self.arbiter_verdict
            )
            if use_html:
        ...
        if self.commit_sha:
            ...
            if use_html:
                lines.append(
                    f"<b>Commit:</b> <code>{escape_html(self.commit_sha[:8])}</code>"
                )
            else:
    ...
    async def _send_with_fallback(
        ...
        for parse_mode, text in [("HTML", html_text), (None, plain_text)]:
            try:
                ...
                if parse_mode == "HTML":
                    logger.warning(
                        "HTML send error, trying plaintext: %s", type(e).__name__
                    )
                    continue
    ...
    async def _edit_message(
        ...
        for parse_mode, use_html in [("HTML", True), (None, False)]:
            try:
                ...
                if parse_mode == "HTML":
                    logger.warning(
                        "HTML edit error, trying plaintext: %s", type(e).__name__
                    )
                    continue
                return False
...
class TelegramNotifier:
    ...
    async def notify_arbiter_decision(
        ...
        if decision.auto_fix_allowed:
            ...
            card.arbiter_verdict = (
                f"🔴 Denied: {safe_truncate(decision.stop_reason or 'N/A', 40)}"
            )
        ...
        if decision.fix_objectives:
            ...
            detail_lines.append(
                f"• Objectives: {escape_html(safe_truncate(objectives_str, 100))}"
            )

        await card.send_detail_reply("\n".join(detail_lines))

    async def notify_fix_started(
        self, job: SupervisorJob, loop_num: int, max_loops: int
    ) -> None:
    ...
    async def notify_error(self, job: SupervisorJob, error: str) -> None:
        ...
        await card.send_detail_reply(f"<b>Error:</b>\n<pre>{escaped_error}</pre>")
