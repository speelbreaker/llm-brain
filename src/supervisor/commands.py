...
class SupervisorCommands:
    ...
    async def handle_command(
        ...
        return CommandResult(
            success=False,
            message=f"Unknown supervisor command: {command}"
        )
    
    async def cmd_autofix(self, pr_number: int, user_id: int, repo: Optional[str] = None) -> CommandResult:
        ...
        approval = self.store.set_pr_approval(repo, pr_number, approved=True, user_id=user_id)
    ...
    async def cmd_revoke(self, pr_number: int, user_id: int, repo: Optional[str] = None) -> CommandResult:
    ...
    async def cmd_pause(self, pr_number: int, user_id: int, repo: Optional[str] = None) -> CommandResult:
        ...
        approval = self.store.set_pr_paused(repo, pr_number, paused=True, user_id=user_id)
    ...
    async def cmd_resume(self, pr_number: int, user_id: int, repo: Optional[str] = None) -> CommandResult:
    ...
    async def cmd_rerun(self, pr_number: int, user_id: int, repo: Optional[str] = None) -> CommandResult:
    ...
    async def _cmd_pr(self, args: list[str]) -> CommandResult:
        ...
        if not args or not args[0].isdigit():
            return CommandResult(success=False, message="Usage: /supervisor pr <number>")
        
        pr_number = int(args[0])
        repo = self._repo
        
        if not repo:
            return CommandResult(success=False, message="No default repository configured")
        ...
        lines.append(f"Autofix approved: {'✅' if approval.approved_by_telegram else '❌'}")
