...
class VolSurfaceConfig(BaseModel):
    ...
    dte_bands: Optional[List[DteBand]] = Field(
        default=None, description="DTE-specific IV multipliers"
    )
    skew: Optional[SkewTemplate] = Field(default=None, description="Skew configuration")

    regime_override: bool = Field(
        default=True, description="Allow regime to override IV multiplier"
    )
    vrp_offset_enabled: bool = Field(
        default=False, description="Add VRP as vol-point offset"
    )
    ...
    def from_calibration(
        cls, recommended_vol_surface: Dict[str, Any]
    ) -> "VolSurfaceConfig":
        """Create VolSurfaceConfig from calibration recommended_vol_surface dict."""
        dte_bands = None
        if (
            "dte_bands" in recommended_vol_surface
            and recommended_vol_surface["dte_bands"]
        ):
...
def update_vol_surface_from_calibration(
