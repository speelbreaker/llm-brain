...
class RegimeParams:
    ...
    skew_template: Dict[str, float] = field(
        default_factory=lambda: DEFAULT_SKEW_TEMPLATE.copy()
    )
    phi_iv: float = 0.9
    phi_skew: float = 0.85
...
def cluster_greg_sensors_from_real_data(
    ...
    if not available_cols:
        raise ValueError(
            f"No valid sensor columns found. Expected any of: {GREG_SENSOR_COLUMNS}"
        )
    ...
    if len(df_work) < n_clusters:
        raise ValueError(
            f"Not enough data points ({len(df_work)}) for {n_clusters} clusters"
        )
...
def fit_regime_params_from_clusters(
    ...
    for regime_id in sorted(df_sensors_with_regime["regime_id"].unique()):
        cluster_data = df_sensors_with_regime[
            df_sensors_with_regime["regime_id"] == regime_id
        ]
...
def _name_regime(regime_id: int, mu_rv: float, mu_vrp: float, phi_iv: float) -> str:
    ...
    vrp_level = (
        "rich_vrp" if mu_vrp > 0.15 else ("cheap_vrp" if mu_vrp < 0.05 else "fair_vrp")
    )
...
def iv_for_delta(
    ...
    sorted_items = sorted(
        [(float(k), v) for k, v in skew_template.items()], key=lambda x: x[0]
    )
...
def load_regime_model(
    underlying: str, path: Optional[Path] = None
...
def load_volatility_history(underlying: str) -> Optional[pd.DataFrame]:
    ...
    """
    filepath = (
        VOLATILITY_HISTORY_DIR / f"{underlying.lower()}_volatility_history.parquet"
    )
