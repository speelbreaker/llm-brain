...
class GregSelectorSensors(BaseModel):
    ...
    vrp_30d: Optional[float] = Field(
        default=None, description="Volatility Risk Premium (IV_30d - RV_30d)"
    )
    vrp_7d: Optional[float] = Field(
        default=None, description="7-day VRP (IV_7d - RV_7d)"
    )
    front_rv_iv_ratio: Optional[float] = Field(
        default=None, description="Front RV/IV ratio (RV_7d / IV_7d)"
    )
    chop_factor_7d: Optional[float] = Field(
        default=None, description="Chop factor (RV_7d / IV_30d), < 0.6 = calm"
    )
    iv_rank_6m: Optional[float] = Field(
        default=None, description="IV Rank over 6 months (0-1 scale)"
    )
    term_structure_spread: Optional[float] = Field(
        default=None, description="IV_7d - IV_30d term structure spread"
    )
    skew_25d: Optional[float] = Field(
        default=None, description="25-delta skew (Put IV - Call IV)"
    )
    adx_14d: Optional[float] = Field(
        default=None, description="Average Directional Index (14-day)"
    )
    rsi_14d: Optional[float] = Field(
        default=None, description="Relative Strength Index (14-day)"
    )
    price_vs_ma200: Optional[float] = Field(
        default=None, description="Current price minus 200-day MA"
    )
    predicted_funding_rate: Optional[float] = Field(
        default=None, description="Predicted perpetual funding rate"
    )
...
class GregSelectorDecision(BaseModel):
    ...
    step_name: str = Field(
        default="", description="Name of the matched step in the waterfall"
    )
    meta: Dict[str, Any] = Field(
        default_factory=dict, description="Metadata from the Greg selector spec"
    )
...
def load_greg_spec() -> Dict[str, Any]:
    ...
    spec_path = (
        ...
        / "docs"
        / "greg_mandolini"
        / "GREG_SELECTOR_RULES_FINAL.json"
    )
...
def get_calibration_spec_with_overrides(
...
def _get_calibration_context(
    spec: Dict[str, Any], env_mode: Optional[str] = None
) -> Dict[str, float]:
    """
...
def _matches_condition(
    ...
    if " OR " in condition:
        ...
        return any(
            _evaluate_simple_condition(p.strip(), sensors, calibration_ctx)
            for p in parts
        )
