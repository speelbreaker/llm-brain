...
class HarvestConfig(BaseModel):
    ...
    data_root: str = Field(
        default="data/live_deribit", description="Root directory for harvester data"
    )
    underlying: Optional[str] = Field(
        default=None, description="Override underlying (defaults to top-level)"
    )
    start_time: Optional[datetime] = Field(
        default=None, description="Inclusive start time filter"
    )
    end_time: Optional[datetime] = Field(
        default=None, description="Inclusive end time filter"
    )
    snapshot_step: int = Field(
        default=1, ge=1, description="Use every N-th snapshot to thin data"
    )
    max_snapshots: Optional[int] = Field(
        default=None, description="Max number of snapshots to process"
    )
...
class BandConfig(BaseModel):
    ...
    max_samples: Optional[int] = Field(
        default=None, description="Override max_samples for this band"
    )
...
class CalibrationFilters(BaseModel):
    ...
    min_mark_price: Optional[float] = Field(
        default=None, description="Minimum mark price to include"
    )
    min_open_interest: Optional[float] = Field(
        default=None, description="Minimum open interest to include"
    )
    min_vega: Optional[float] = Field(
        default=None, description="Minimum vega to include"
    )
...
class CalibrationConfig(BaseModel):
    ...
    iv_multiplier: float = Field(
        default=1.0, ge=0.1, le=10.0, description="IV multiplier for synthetic pricing"
    )
    default_iv: float = Field(
        default=0.6, ge=0.01, le=3.0, description="Default IV when RV unavailable"
    )
    rv_window_days: int = Field(
        default=7,
        ge=1,
        le=365,
        description="Window for realized volatility computation",
    )
    risk_free_rate: float = Field(
        default=0.0, description="Risk-free rate for Black-Scholes"
    )
    max_samples: int = Field(default=80, ge=1, description="Maximum samples to process")
    skew: Optional[SkewConfig] = Field(
        default_factory=SkewConfig, description="Skew configuration"
    )

    source: Literal["live", "harvested"] = Field(
        default="live", description="Data source: live API or harvested Parquet"
    )
    harvest: Optional[HarvestConfig] = Field(
        default=None, description="Harvest configuration (when source='harvested')"
    )
    ...
    bands: Optional[List[BandConfig]] = Field(
        default=None, description="Optional DTE bands for multi-band calibration"
    )
    bucket_by_dte: Optional[List[Tuple[float, float]]] = Field(
        default=None, description="DTE buckets for metrics [(min, max), ...]"
    )
    bucket_by_abs_delta: Optional[List[Tuple[float, float]]] = Field(
        default=None, description="Absolute delta buckets [(min, max), ...]"
    )

    filters: Optional[CalibrationFilters] = Field(
        default=None, description="Liquidity filters"
    )

    fit_skew: bool = Field(
        default=False, description="Whether to fit skew anchor ratios"
    )
    return_rows: bool = Field(
        default=True, description="Whether to return individual option rows"
    )
    emit_recommended_vol_surface: bool = Field(
        default=True, description="Whether to emit recommended vol_surface snippet"
    )
...
class DteBandResult(BaseModel):
    ...
    vega_weighted_mae_pct: Optional[float] = Field(
        default=None, description="Vega-weighted MAE percentage for the band"
    )
...
class GlobalMetrics(BaseModel):
    ...
    mae_vol_points: Optional[float] = Field(
        default=None, description="MAE in absolute vol points"
    )
    vega_weighted_mae_pct: Optional[float] = Field(
        default=None, description="Vega-weighted MAE percentage"
    )
...
class SkewFitResult(BaseModel):
    ...
    anchor_ratios: Dict[str, float] = Field(
        ..., description="Fitted anchor ratios by delta string"
    )
    min_dte: float
    max_dte: float
...
class ExtendedCalibrationResult(BaseModel):
    ...
    rv_source: Optional[str] = Field(
        default=None, description="Source of RV: 'deribit_history' or 'default_iv'"
    )
    atm_iv: Optional[float] = None
    atm_source: Optional[str] = Field(
        default=None,
        description="Source of ATM IV: 'delta', 'nearest_strike', or 'none'",
    )
    recommended_iv_multiplier: Optional[float] = None
    ...
    option_types_used: Optional[List[str]] = Field(
        default=None, description="Option types included: ['C'], ['P'], or ['C', 'P']"
    )
    by_option_type: Optional[Dict[str, OptionTypeMetrics]] = Field(
        default=None, description="Metrics split by option type"
    )
...
class ReproducibilityMetadata(BaseModel):
    ...
    harvest_config: Optional[Dict[str, Any]] = Field(
        default=None, description="Harvest configuration used"
    )
    calibration_config_hash: Optional[str] = Field(
        default=None, description="SHA-256 hash of calibration config"
    )
    greg_regimes_version: Optional[Dict[str, Any]] = Field(
        default=None, description="Version info for greg_regimes.json"
    )
...
class HistoricalCalibrationResult(ExtendedCalibrationResult):
    ...
    data_quality: Optional[DataQualityBlock] = Field(
        default=None, description="Data quality summary"
    )
    reproducibility: Optional[ReproducibilityMetadata] = Field(
