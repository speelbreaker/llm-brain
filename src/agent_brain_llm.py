...
def validate_llm_decision(
    ...
    if action == ActionType.OPEN_COVERED_CALL.value:
        ...
        if not symbol:
            raise ValueError(
                "invalid_symbol_reference: symbol required for OPEN_COVERED_CALL"
            )
        ...
        if not from_symbol or not to_symbol:
            raise ValueError(
                "invalid_symbol_reference: from_symbol and to_symbol required for ROLL"
            )
        if from_symbol not in open_position_symbols:
            raise ValueError(
                f"invalid_symbol_reference: '{from_symbol}' not in open positions"
            )
        if to_symbol not in candidate_symbols:
            raise ValueError(
                f"invalid_symbol_reference: '{to_symbol}' not in candidates"
            )
        ...
        if not symbol:
            raise ValueError(
                "invalid_symbol_reference: symbol required for CLOSE_COVERED_CALL"
            )
        if symbol not in open_position_symbols:
            raise ValueError(
                f"invalid_symbol_reference: '{symbol}' not in open positions"
            )
...
def choose_action_with_llm(
    ...
    if is_training:
        compact_state["max_calls_per_underlying"] = (
            settings.max_calls_per_underlying_training
        )
...
Return ONLY valid JSON matching the requested schema."""
    ...
    try:
        ...
        if settings.llm_validation_strict:
            ...
            decision["params"] = {}
            decision["reasoning"] = (
                f"Validation warning (downgraded to DO_NOTHING): {e}"
            )
