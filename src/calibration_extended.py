...
def compute_bucket_metrics(
    ...
    pct_gt_10 = (
        sum(1 for e in errors if e > 10.0) / len(errors) * 100 if errors else 0.0
    )
    ...
    if bucket_by_dte:
        for dte_min, dte_max in bucket_by_dte:
            ...
            if bucket_rows:
                ...
                avg_mark = np.mean(
                    [r.get("mark_iv", 0.0) for r in bucket_rows if r.get("mark_iv")]
                )
...
def compute_vega_weighted_mae(rows: List[Dict[str, Any]]) -> Optional[float]:
    ...
    """
    vega_rows = [
        (r, r.get("vega", 0.0)) for r in rows if r.get("vega") and r.get("vega") > 0
    ]
    if not vega_rows:
        return None
        ...
        dte_bands = [
            {
                ...
                "min_dte": b.min_dte,
                "max_dte": b.max_dte,
                "iv_multiplier": b.recommended_iv_multiplier
                or recommended_iv_multiplier,
            }
...
def run_calibration_extended(
    ...
    spot_history = get_spot_history_for_rv(
        underlying, as_of=now, window_days=config.rv_window_days
    )
    ...
    rows: List[Dict[str, Any]] = []
    skew_enabled = (
        config.skew.enabled if config.skew else settings.synthetic_skew_enabled
    )
    skew_min_dte = (
        config.skew.min_dte if config.skew else settings.synthetic_skew_min_dte
    )
    skew_max_dte = (
        config.skew.max_dte if config.skew else settings.synthetic_skew_max_dte
    )
    ...
    for band in bands_to_use:
        ...
        if band_rows:
            ...
            avg_mark = np.mean(
                [r.get("mark_iv", 0.0) for r in band_rows if r.get("mark_iv")]
            )
            rec_mult = None
            if rv_annualized > 0 and avg_mark and avg_mark > 0:
    ...
    for opt_type in option_types_used:
        ...
        for band in bands_to_use:
            band_rows = [
                r for r in type_rows if band.min_dte <= r.get("dte", 0) <= band.max_dte
            ]
            if band_rows:
                band_errors = [abs(r.get("diff_pct", 0.0)) for r in band_rows]
                mae = float(np.mean(band_errors))
    ...
    if config.fit_skew:
        skew_result = fit_skew_anchor_ratios(rows, rv_annualized, config.iv_multiplier)
        if skew_result:
            current_ratios = {
                "0.15": settings.synthetic_skew_anchor_15
                if hasattr(settings, "synthetic_skew_anchor_15")
                else 1.0,
                "0.25": settings.synthetic_skew_anchor_25
                if hasattr(settings, "synthetic_skew_anchor_25")
                else 1.0,
                "0.35": settings.synthetic_skew_anchor_35
                if hasattr(settings, "synthetic_skew_anchor_35")
                else 1.0,
            }
    ...
    if config.emit_recommended_vol_surface and recommended_iv_multiplier:
        ...
        vol_surface_diff = VolSurfaceDiff(
            iv_multiplier_delta=round(
                recommended_iv_multiplier - config.iv_multiplier, 4
            ),
...
def run_historical_calibration_from_harvest(
    ...
    reproducibility = ReproducibilityMetadata(
        harvest_config={
            "data_root": harvest.data_root,
            "underlying": underlying,
            "start_time": harvest.start_time.isoformat()
            if harvest.start_time
            else None,
    ...
    if (
        "dte_days" not in df.columns
        and "expiry_timestamp" in df.columns
        and "harvest_time" in df.columns
    ):
        df["dte_days"] = (
            pd.to_datetime(df["expiry_timestamp"], unit="s", utc=True)
            - df["harvest_time"]
        ).dt.total_seconds() / 86400.0

    mask = (df["dte_days"] >= config.min_dte) & (df["dte_days"] <= config.max_dte)
    ...
    for snap_time in snapshot_times:
        ...
        if spot_series:
            rv = compute_realized_volatility(
                spot_series, snap_time, config.rv_window_days
            )
        else:
        ...
        for _, row in snap_df.iterrows():
            ...
            if is_call:
                syn_price_usd = black_scholes_call_price(
                    spot, strike, t_years, sigma, 0.0
                )
            else:
                syn_price_usd = black_scholes_put_price(
                    spot, strike, t_years, sigma, 0.0
                )
    ...
    avg_spot = (
        df["underlying_price"].mean() if "underlying_price" in df.columns else 0.0
    )
    ...
    for band in bands_to_use:
        band_rows = [
            r for r in all_rows if band.min_dte <= r.get("dte", 0) <= band.max_dte
        ]
        if band_rows:
            band_errors = [abs(r.get("diff_pct", 0.0)) for r in band_rows]
            mae = float(np.mean(band_errors))
...
def build_vol_surface_from_calibration(
    ...
    return {
        ...
        "skew": {
            "enabled": True,
            "anchor_ratios": calibration_result.recommended_skew.anchor_ratios
            if calibration_result.recommended_skew
            else {},
        }
        if calibration_result.recommended_skew
        else None,
