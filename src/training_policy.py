...
def build_training_actions(
    ...
    for underlying in cfg.underlyings:
        ...
        remaining_slots = (
            cfg.max_calls_per_underlying_training - opened_for_this_underlying
        )
        ...
        if cfg.training_profile_mode == "ladder":
            ...
            for profile_name in cfg.training_strategies:
                ...
                available_for_profile = [
                    c for c in available_cands if c.symbol not in used_symbols
                ]
                if not available_for_profile:
                    continue

                candidate_result = pick_candidate_for_profile(
...
def summarize_training_actions(actions: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for a in actions:
        ...
        summary["by_underlying"][underlying] = (
            summary["by_underlying"].get(underlying, 0) + 1
        )
