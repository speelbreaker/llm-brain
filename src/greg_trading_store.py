...
class GregTradingState:
    ...
    strategy_live_enabled: Dict[str, bool] = field(
...
class GregTradingStore:
    ...
    def get_state(self) -> Dict:
        ...
        with self._lock:
            return {
                ...
                if self._state.last_mode_change
                else None,
        ...
        with self._lock:
            ...
            if mode == GregTradingMode.LIVE:
                ...
                if not self._state.strategy_live_enabled.get(strategy, False):
                    return (
                        False,
                        f"Strategy {strategy} is not enabled for live execution",
                    )
    ...
    def atomic_execute_check(
        self, strategy: str, deribit_env: str
    ) -> Tuple[bool, str, bool]:
        """
        ...
        with self._lock:
            ...
            if mode == GregTradingMode.PAPER:
                if deribit_env != "testnet":
                    return (
                        False,
                        f"PAPER mode requires testnet (current: {deribit_env})",
                        True,
                    )
            ...
            if mode == GregTradingMode.LIVE:
                if deribit_env != "mainnet":
                    return (
                        False,
                        f"LIVE mode requires mainnet (current: {deribit_env})",
                        False,
                    )
    ...
    def init_from_settings(self) -> None:
        ...
        with self._lock:
            ...
            self._state.max_notional_per_position = (
                settings.greg_live_max_notional_usd_per_position
            )
            self._state.max_notional_per_underlying = (
                settings.greg_live_max_notional_usd_per_underlying
            )
    ...
    def check_notional_limits(
        self, position_notional: float, current_underlying_exposure: float
    ) -> Tuple[bool, str]:
        """
        ...
        with self._lock:
            ...
            if position_notional > max_pos:
                return (
                    False,
                    f"Position notional ${position_notional:.0f} exceeds limit ${max_pos:.0f}",
                )
            ...
            if new_exposure > max_und:
                return (
                    False,
                    f"Total underlying exposure ${new_exposure:.0f} would exceed limit ${max_und:.0f}",
                )
