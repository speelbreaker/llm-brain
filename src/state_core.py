...
def _calculate_dte_float(
    expiry: datetime, reference_time: Optional[datetime] = None
) -> float:
...
def build_agent_state_from_raw(
    ...
    *,
    ...
    for opt in raw.options:
        ...
        rv = (
            opt.rv
            if opt.rv is not None
            else raw.realized_vol.get(opt.underlying, opt.mark_iv * 0.8)
        )
