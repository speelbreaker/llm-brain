...
class RealScraperDataSource(MarketDataSource):
    ...
    def _ensure_loaded(self) -> pd.DataFrame:
        ...
        if self._df is None:
            ...
            if (
                not self._df.empty
                and "harvest_time" in self._df.columns
                and "underlying_price" in self._df.columns
            ):
                for _, row in self._df.drop_duplicates(
                    subset=["harvest_time"]
                ).iterrows():
                    if pd.notna(row["harvest_time"]) and pd.notna(
                        row["underlying_price"]
                    ):
                        self._spot_cache[row["harvest_time"]] = float(
                            row["underlying_price"]
                        )
    ...
    def get_spot_ohlc(
        ...
        unique_times = filtered.drop_duplicates(subset=["harvest_time"])[
            ["harvest_time", "underlying_price"]
        ]
        ...
        result = pd.DataFrame(
            ...
            index=pd.DatetimeIndex(
                unique_times["harvest_time"].values, name="timestamp"
            ),
        )
        ...
        result = pd.DataFrame(
            {
                ...
                "volume": filtered.get(
                    "volume", pd.Series([0.0] * len(filtered))
                ).values,
            },
    ...
    def get_spot_price(self, as_of: datetime) -> Optional[float]:
        ...
        if self._spot_cache:
            ...
            closest_time = min(
                cache_times, key=lambda t: abs((t - as_of_utc).total_seconds())
            )
