    ...
    for k, v in d.items():
        if isinstance(v, float):
            ...
            result[k] = [
                ...
                else (_sanitize_dict(x) if isinstance(x, dict) else x)
                for x in v
            ]
        else:
            result[k] = v
...
def _compute_equity_curve(
    ...
    for trade in sorted_trades:
        ...
        if spot_now is None:
            closest_time = min(
                sorted_times, key=lambda t: abs((t - close_time).total_seconds())
            )
    ...
    if sorted_trades:
        ...
        if end_time > last_trade_time:
            ...
            if spot_end is None:
                closest_time = min(
                    sorted_times, key=lambda t: abs((t - end_time).total_seconds())
                )
...
def _compute_enhanced_metrics(
    ...
    net_profit_pct = (
        (net_profit_usd / initial_equity * 100) if initial_equity > 0 else 0.0
    )

    hodl_profit_usd = final_hodl - initial_equity
    hodl_profit_pct = (
        (hodl_profit_usd / initial_equity * 100) if initial_equity > 0 else 0.0
    )
    ...
    if len(pct_returns) > 1:
        variance = sum((r - mean_return) ** 2 for r in pct_returns) / (
            len(pct_returns) - 1
        )
    ...
    if len(downside_returns) > 1:
        downside_variance = sum(r**2 for r in downside_returns) / (
...
class BacktestManager:
    ...
    def get_status(self) -> Dict[str, Any]:
        with self._lock:
            ...
            status_dict = {
                ...
                "finished_at": self._status.finished_at.isoformat()
                if self._status.finished_at
                else None,
                "progress_pct": _sanitize_float(self._status.progress_pct),
                "current_time": self._status.current_time.isoformat()
                if self._status.current_time
                else None,
                ...
                "start_date": self._status.start_date.isoformat()
                if self._status.start_date
                else None,
                "end_date": self._status.end_date.isoformat()
                if self._status.end_date
                else None,
                "metrics": _sanitize_dict(self._status.metrics)
                if self._status.metrics
                else {},
                "recent_steps": [
                    {
    ...
    def _save_run_to_store(
        self, failed: bool = False, error: Optional[str] = None
    ) -> None:
        ...
        try:
            ...
            for style, curve in getattr(self, "_all_equity_curves", {}).items():
                equity_curves_json[style] = [
                    [
                        pt.time.isoformat(),
                        _sanitize_float(pt.equity),
                        _sanitize_float(pt.hodl_equity),
                    ]
                    for pt in curve
                ]
            ...
            result = BacktestRunResult(
                run_id=run_id,
                created_at=self._status.started_at.isoformat()
                if self._status.started_at
                else "",
                status="finished",
                config=self._status.config,
            ...
            if self._status.started_at and self._status.finished_at:
                result.duration_seconds = (
                    self._status.finished_at - self._status.started_at
                ).total_seconds()
    ...
    def _export_training_data_if_enabled(
        ...
        try:
            ...
            if not examples:
                print(
                    f"[TrainingExport] No training examples generated for {underlying}"
                )
                return
            ...
            print(
                f"[TrainingExport] Saved {stats['total_examples']} examples to {csv_path} and {jsonl_path}"
            )
    ...
    def _export_candidate_level_data_if_enabled(
        ...
        try:
            ...
            for style in styles_to_export:
                examples = build_candidate_level_examples(
                    decision_steps, style, include_empty_steps=True
                )

                if not examples:
                    print(
                        f"[CandidateExport] No candidate examples for {underlying} ({style})"
                    )
                    continue
                ...
                print(
                    f"[CandidateExport] Saved {stats['total_examples']} candidate examples to {csv_path}"
                )
    ...
    def start(
        ...
        def worker() -> None:
            try:
                ...
                interval_bars = max(
                    1, int(decision_interval_hours / bar_duration_hours)
                )
                ...
                use_live_deribit_source = (
                    chain_mode == "live_chain" and end_date_naive < now_utc
                )

                if use_live_deribit_source:
                    underlying_dir = (
                        f"{underlying}_USDC"
                        if "_USDC" not in underlying
                        else underlying
                    )
                    ...
                    if snapshot_count == 0:
                        print(
                            f"[BacktestManager] WARNING: No harvested data found for {underlying_dir} in date range"
                        )
                    else:
                        print(
                            f"[BacktestManager] Using LiveDeribitDataSource with {snapshot_count} rows ({underlying_dir})"
                        )
                else:
                    ds = DeribitDataSource()
                    print("[BacktestManager] Using DeribitDataSource (real-time API)")
                    ...
                    for idx, t in enumerate(decision_times):
                        ...
                        try:
                            ...
                            state = build_historical_state(
                                ds, config, t, collect_debug_samples=should_collect
                            )
                        ...
                        if not step_data.candidates:
                            ...
                            for sc, opt, feats in scored_with_feats:
                                dte_days = (
                                    (opt.expiry - t).total_seconds() / 86400.0
                                    if opt.expiry
                                    else 0.0
                                )
                        ...
                        if best_score >= min_score:
                            try:
                                if current_exit_style == "hold_to_expiry":
                                    trade = sim._simulate_call_hold_to_expiry(
                                        t, best_opt
                                    )
                                else:
                        ...
                        if trade is not None:
                            ...
                            if current_exit_style == "hold_to_expiry":
                                step_data.chosen_hold_to_expiry = (
                                    best_opt.instrument_name
                                )
