...
def load_real_scraper_data(
    ...
    if not files:
        print(
            f"[RealScraperLoader] No data files found for {underlying} "
    ...
    if "harvest_time" in combined.columns:
        ...
        start_utc = (
            start_ts if start_ts.tzinfo else start_ts.replace(tzinfo=timezone.utc)
        )
        end_utc = end_ts if end_ts.tzinfo else end_ts.replace(tzinfo=timezone.utc)

        mask = (combined["harvest_time"] >= start_utc) & (
            combined["harvest_time"] <= end_utc
        )
    ...
    print(
        f"[RealScraperLoader] Loaded {len(combined)} rows, "
