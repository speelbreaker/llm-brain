...
def find_sweet_spots(
    ...
    for cell in cells:
        if (
            ...
            and cell.strategy_pass_frac >= min_pass_frac
        ):
...
def run_full_heatmap_analysis(
    ...
    for underlying in underlyings:
        ...
        samples = list(
            load_metric_samples_for_underlying(underlying, lookback_days=lookback_days)
        )
        ...
        for x_metric, y_metric in metric_pairs:
            ...
            for strategy_key in strategy_keys:
                ...
                sweet_spots = find_sweet_spots(strat_cells, min_env_frac, min_pass_frac)
...
def save_sweetspots_json(
    summaries: List[StrategySweetSpotSummary], output_path: Path
) -> None:
...
def save_sweetspots_markdown(
    ...
    lines.append("Generated from synthetic universe analysis.\n")
        ...
        for strategy in sorted(by_strategy.keys()):
            ...
            for s in summaries_s:
                ...
                for i, sp in enumerate(s.sweet_spots, 1):
                    ...
                    lines.append(
                        f"| {i} | {x_range} | {y_range} | {occ} | {pass_r} | {score} |\n"
                    )
