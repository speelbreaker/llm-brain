...
if TYPE_CHECKING:
    from src.synthetic.regimes import RegimeParams
...
def bs_call_price(
    spot: float, strike: float, t_years: float, sigma: float, r: float = 0.0
) -> float:
    """
    ...
    d1 = (log(spot / strike) + (r + 0.5 * sigma * sigma) * t_years) / (
        sigma * sqrt(t_years)
    )
...
def compute_realized_volatility(
    prices: List[Tuple[datetime, float]], as_of: datetime, window_days: int = 30
) -> float:
    """
...
def _load_volatility_history_for_replay(underlying: str) -> Optional["pd.DataFrame"]:
    ...
    filepath = (
        Path("data/volatility_history")
        / f"{underlying.lower()}_volatility_history.parquet"
    )
...
def get_synthetic_iv(
    ...
    rv = compute_realized_volatility(
        spot_history, as_of, config.synthetic_rv_window_days
    )
