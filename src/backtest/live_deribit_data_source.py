...
class LiveDeribitDataSource:
    ...
    def _ensure_loaded(self) -> None:
        ...
        if "harvest_time" in self._df.columns:
            self._df["harvest_time"] = pd.to_datetime(
                self._df["harvest_time"], utc=True
            )
            self._snapshot_times = sorted(self._df["harvest_time"].unique().tolist())
        else:
            self._snapshot_times = []

    def _find_closest_snapshot_time(
        self, as_of: datetime, tolerance_minutes: int = 60
    ) -> Optional[datetime]:
        ...
        as_of_ts = (
            as_of.timestamp()
            if as_of.tzinfo
            else as_of.replace(tzinfo=timezone.utc).timestamp()
        )
        ...
        for snap_time in self._snapshot_times:
            snap_ts = (
                ...
                else pd.Timestamp(snap_time).timestamp()
            )
    ...
    def _build_spot_ohlc_cache(self) -> None:
        ...
        if self._df is None or self._df.empty:
            self._spot_ohlc_cache = pd.DataFrame(
                columns=["open", "high", "low", "close", "volume"]
            )
            return

        if (
            "harvest_time" not in self._df.columns
            or "underlying_price" not in self._df.columns
        ):
            self._spot_ohlc_cache = pd.DataFrame(
                columns=["open", "high", "low", "close", "volume"]
            )
        ...
        spot_df = (
            self._df.groupby("harvest_time")["underlying_price"]
            .agg(
                open="first",
                high="max",
                low="min",
    ...
    def get_spot_ohlc(
        ...
        mask = (self._spot_ohlc_cache.index >= start_utc) & (
            self._spot_ohlc_cache.index <= end_utc
        )
            ...
            try:
                ...
                if pd.notna(expiry_ts):
                    expiry_dt = datetime.fromtimestamp(
                        float(expiry_ts), tz=timezone.utc
                    )
    ...
    def get_option_ohlc(
        ...
        ohlc = (
            df.groupby("harvest_time")["mark_price"]
            .agg(
                open="first",
                high="max",
                low="min",
