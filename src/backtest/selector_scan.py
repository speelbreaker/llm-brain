...
class SelectorScanResult(BaseModel):
    ...
    total_steps: Dict[str, int] = Field(
        default_factory=dict, description="Total decision points per underlying"
    )
...
class SelectorHeatmapResult(BaseModel):
    ...
    values: List[List[float]] = Field(
        default_factory=list, description="values[y_index][x_index] = pass_pct (0-1)"
    )
...
class EnvironmentHeatmapResult(BaseModel):
    ...
    grid: List[List[float]] = Field(
        default_factory=list, description="grid[y_index][x_index] = % occupancy (0-100)"
    )
...
def _generate_synthetic_sensors(
    ...
    term_structure_spread = term_spread_base + 2.0 * math.sin(
        2 * math.pi * step_index / 30
    )
        ...
        for path_idx in range(config.num_paths):
            ...
            for step_idx in range(num_steps):
                ...
                for strategy_key in GREG_STRATEGIES:
                    if _check_strategy_passes(
                        sensors, strategy_key, config.threshold_overrides
                    ):
...
def compute_environment_heatmap(
    req: EnvironmentHeatmapRequest,
) -> EnvironmentHeatmapResult:
    """
    ...
    seed = hash((req.underlying, req.horizon_days, req.x_metric, req.y_metric)) % (
        2**31
    )
    rng = np.random.default_rng(seed)
    ...
    if total_in_range == 0:
        ...
        occupancy = [
            [round((c / total_in_range) * 100.0, 1) for c in row] for row in counts
        ]
