...
class OptionInstrument(BaseModel):
    ...
    symbol: str = Field(
        ..., description="Deribit instrument name, e.g. BTC-27DEC24-100000-C"
    )
    underlying: str = Field(..., description="Underlying asset: BTC or ETH")
    ...
    mark_iv: Optional[float] = Field(
        default=None, description="Mark implied volatility"
    )
...
class OptionPosition(BaseModel):
    ...
    size: float = Field(
        ..., description="Position size (positive for long, can be negative for short)"
    )
...
class PortfolioState(BaseModel):
    ...
    margin_available_usd: float = Field(
        default=0.0, description="Available margin in USD"
    )
    margin_used_pct: float = Field(
        default=0.0, description="Margin utilization percentage"
    )
...
class VolState(BaseModel):
    ...
    btc_skew: float = Field(
        default=0.0, description="BTC volatility skew (placeholder)"
    )
    ...
    eth_skew: float = Field(
        default=0.0, description="ETH volatility skew (placeholder)"
    )
...
class AgentState(BaseModel):
    ...
    timestamp: datetime = Field(
        default_factory=datetime.utcnow, description="State timestamp"
    )

    underlyings: list[str] = Field(
        default_factory=list, description="List of underlyings"
    )
