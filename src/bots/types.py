...
class StrategyCriterion(BaseModel):
    ...
    value: Optional[float] = Field(
        None, description="Current numeric value, if available"
    )
    min: Optional[float] = Field(None, description="Minimum threshold")
    max: Optional[float] = Field(None, description="Maximum threshold")
    ok: bool = Field(
        ..., description="True if condition passed, False if failed or missing"
    )
    note: Optional[str] = Field(
        None, description="Short explanation: missing_data, below_min, etc."
    )
...
class StrategyEvaluation(BaseModel):
    ...
    strategy_key: str = Field(
        ..., description="Stable identifier (e.g., atm_straddle_30d)"
    )
    ...
    summary: str = Field(
        ..., description="1-2 sentence explanation of why it passes or doesn't"
    )
    criteria: List[StrategyCriterion] = Field(
        default_factory=list, description="List of metric checks"
    )
