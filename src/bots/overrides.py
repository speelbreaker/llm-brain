...
class GlobalRiskOverrides(BaseModel):
    ...
    max_margin_pct: Optional[float] = Field(
        default=None, description="Override max margin usage percentage"
    )
    max_net_delta: Optional[float] = Field(
        default=None, description="Override max absolute net delta"
    )
    daily_drawdown_limit_pct: Optional[float] = Field(
        default=None, description="Override daily drawdown limit percentage"
    )
    liquidity_max_spread_pct: Optional[float] = Field(
        default=None, description="Override max bid/ask spread percentage"
    )
    liquidity_min_open_interest: Optional[int] = Field(
        default=None, description="Override min open interest requirement"
    )
...
class BotRiskOverrides(BaseModel):
    ...
    max_equity_share: Optional[float] = Field(
        default=None, description="Max percentage of equity this bot can use"
    )
    max_positions_per_underlying: Optional[int] = Field(
        default=None, description="Max positions per underlying for this bot"
    )
    max_notional_usd_per_position: Optional[float] = Field(
        default=None, description="Max notional USD per position"
    )
    max_notional_usd_per_underlying: Optional[float] = Field(
        default=None, description="Max notional USD per underlying"
    )
...
class EntryRuleOverrides(BaseModel):
    ...
    thresholds: Dict[str, float] = Field(
        default_factory=dict, description="Map of threshold key to override value"
    )
...
class BotOverrideConfig(BaseModel):
    ...
    use_global_risk_overrides: bool = Field(
        default=False, description="Enable global risk overrides"
    )
    use_bot_risk_overrides: bool = Field(
        default=False, description="Enable per-bot risk overrides"
    )
    use_entry_rule_overrides: bool = Field(
        default=False, description="Enable entry rule threshold overrides"
    )
    global_risk: GlobalRiskOverrides = Field(default_factory=GlobalRiskOverrides)
    bots: Dict[str, BotRiskOverrides] = Field(
        default_factory=dict, description="Per-bot risk overrides keyed by bot_id"
    )
    entry_rules: Dict[str, EntryRuleOverrides] = Field(
        default_factory=dict, description="Per-bot entry rule overrides keyed by bot_id"
    )
    ...
    try:
        ...
        for key, value in calibration.items():
            if isinstance(value, (int, float)):
                label, unit = threshold_labels.get(
...
def get_entry_rules_for_ui(
    ...
    use_overrides = (
        env_mode == EnvironmentMode.TEST and overrides.use_entry_rule_overrides
    )
...
def get_global_risk_for_ui(env_mode: EnvironmentMode) -> Dict[str, Any]:
    ...
    overrides = load_overrides(env_mode)
    use_overrides = (
        env_mode == EnvironmentMode.TEST and overrides.use_global_risk_overrides
    )
    ...
    fields = [
        {
            ...
            "current_value": overrides.global_risk.max_margin_pct
            if use_overrides and overrides.global_risk.max_margin_pct is not None
            else settings.max_margin_used_pct,
            "unit": "%",
        },
        {
            "key": "max_net_delta",
            ...
            "current_value": overrides.global_risk.max_net_delta
            if use_overrides and overrides.global_risk.max_net_delta is not None
            else settings.max_net_delta_abs,
            "unit": "",
        },
        {
            ...
            "current_value": overrides.global_risk.daily_drawdown_limit_pct
            if use_overrides
            and overrides.global_risk.daily_drawdown_limit_pct is not None
            else settings.daily_drawdown_limit_pct,
            "unit": "%",
        },
        {
            ...
            "current_value": overrides.global_risk.liquidity_max_spread_pct
            if use_overrides
            and overrides.global_risk.liquidity_max_spread_pct is not None
            else settings.liquidity_max_spread_pct,
            "unit": "%",
        },
        {
            ...
            "current_value": overrides.global_risk.liquidity_min_open_interest
            if use_overrides
            and overrides.global_risk.liquidity_min_open_interest is not None
            else settings.liquidity_min_open_interest,
            "unit": "contracts",
        },
    ]
...
def get_bot_risk_for_ui(bot_id: str, env_mode: EnvironmentMode) -> Dict[str, Any]:
    ...
    overrides = load_overrides(env_mode)
    use_overrides = (
        env_mode == EnvironmentMode.TEST and overrides.use_bot_risk_overrides
    )
    ...
    fields = [
        {
            ...
            "default_value": 25.0,
            "current_value": bot_overrides.max_equity_share
            if use_overrides and bot_overrides.max_equity_share is not None
            else 25.0,
            "unit": "%",
        },
        {
            ...
            "current_value": bot_overrides.max_notional_usd_per_position
            if use_overrides and bot_overrides.max_notional_usd_per_position is not None
            else settings.greg_live_max_notional_usd_per_position,
            "unit": "USD",
        },
        {
            ...
            "current_value": bot_overrides.max_notional_usd_per_underlying
            if use_overrides
            and bot_overrides.max_notional_usd_per_underlying is not None
            else settings.greg_live_max_notional_usd_per_underlying,
            "unit": "USD",
        },
        {
            ...
            "default_value": 3,
            "current_value": bot_overrides.max_positions_per_underlying
            if use_overrides and bot_overrides.max_positions_per_underlying is not None
            else 3,
            "unit": "",
        },
    ]
