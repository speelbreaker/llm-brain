...
def build_rules_summary(cfg: ResolvedBacktestConfig) -> Dict[str, Any]:
    ...
    if t.dte_range is not None:
        subtitle_parts.append(f"DTE range: {t.dte_range[0]}-{t.dte_range[1]} days")
    ...
    if r.require_ivrv_filter is not None:
        badges.append(
            {"label": "IV/RV filter", "value": _on_off(r.require_ivrv_filter)}
        )
        ...
        filter_items.append(
            {
                ...
                "detail": (
                    f"Min IV/RV: {t.min_ivrv:.2f}" if t.min_ivrv is not None else ""
                ),
            }
        )
