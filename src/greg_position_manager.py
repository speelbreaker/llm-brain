...
def _evaluate_straddle_strangle(
    ...
    profit_take_pct = rules.get_calibration(
        "straddle_profit_take_pct" if is_straddle else "strangle_profit_take_pct", 0.50
    )
    ...
    close_at_dte = int(
        rules.get_calibration(
            "straddle_close_at_dte" if is_straddle else "strangle_close_at_dte", 21
        )
    )
...
def _evaluate_credit_spread(
    ...
    touched = (is_bull_put and spot_price <= short_strike) or (
        not is_bull_put and spot_price >= short_strike
    )
...
def _evaluate_mock_position(
    ...
    if "IRON" in strategy_code and (
        "FLY" in strategy_code or "BUTTERFLY" in strategy_code
    ):
