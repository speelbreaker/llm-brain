    ...
    try:
        result = subprocess.run(
            [
                "node",
                "-e",
                'console.log(require("@openai/codex/package.json").version)',
            ],
            capture_output=True,
            text=True,
            timeout=10,
        ...
        if result.returncode == 0:
            ...
            return (
                True,
                f"Codex CLI v{version} installed (but sandbox conflicts with Replit)",
            )
        else:
...
def get_codex_status() -> str:
    ...
    try:
        ...
        node_version = (
            node_result.stdout.strip() if node_result.returncode == 0 else "not found"
        )
    except Exception:
        node_version = "error"
    ...
    try:
        npm_result = subprocess.run(
            ["npm", "-v"], capture_output=True, text=True, timeout=5
        )
        npm_version = (
            npm_result.stdout.strip() if npm_result.returncode == 0 else "not found"
        )
    except Exception:
        npm_version = "error"
