...
class LLMReviewResult:
    ...
    def error_result(cls, error: str, raw_response: str = "") -> "LLMReviewResult":
        return cls(
            summary=[f"Review error: {error[:200]}"]
            if error
            else ["Error during review"],
...
class LLMClient:
        ...
        for model in models:
            try:
                ...
                if not content.strip() and retry_on_empty:
                    logger.warning(
                        f"Empty response from {model}, retrying with higher tokens..."
                    )
                    retry_tokens = min(max_tokens * 2, 20000)
                    retry_effort = (
                        "high"
                        if self._reasoning_effort == "xhigh"
                    ...
                    if not content.strip():
                        logger.warning("Still empty after retry, trying next model...")
                        continue
    ...
    def review_changes(
        ...
        try:
            ...
            if not content.strip():
                _save_artifact(
                    run_id, "llm_empty.txt", f"Model: {model_used}\nResponse was empty"
                )
                return LLMReviewResult(
                    summary=[
                        "Model returned empty response - no issues detected or review failed"
                    ],
                    overall_severity="INFO",
