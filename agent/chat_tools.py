...
SECRET_PATTERNS = [
    ...
    (
        re.compile(
            r'(secret\s*[=:]\s*["\']?)([a-zA-Z0-9_\-]{16,})(["\']?)', re.IGNORECASE
        ),
        r"\1***REDACTED***\3",
    ),
    (
...
def get_status() -> ToolResult:
    ...
    try:
        ...
        if review:
            status_parts.append(
                f"Last review: {review.created_at} - {review.overall_severity}"
            )
    ...
    try:
        ...
        for d in important_dirs:
            ...
            if path.exists() and path.is_dir():
                py_files = [f for f in path.rglob("*.py") if not should_ignore_path(f)][
                    :15
                ]
        ...
        return ToolResult(
            ...
            output="\n".join(project_map)
            if project_map
            else "No recognized project structure found.",
        )
    except Exception as e:
...
def run_smoke_tests() -> ToolResult:
    ...
    try:
        ...
        return ToolResult(
            success=False, output="Smoke tests timed out after 2 minutes."
        )
    except Exception as e:
...
def search_repo(
    query: str, limit: int = 20, include_context: bool = True
) -> ToolResult:
    """
    ...
    try:
        if not query or len(query) < 2:
            return ToolResult(
                success=False, output="Query must be at least 2 characters."
            )
        ...
        cmd = (
            ["grep", "-rni"] + context_args + include_args + exclude_args + [query, "."]
        )
...
def open_file(path: str, start_line: int = 1, end_line: int = 50) -> ToolResult:
    ...
    try:
        ...
        header = (
            f"📄 {path} (lines {start_line}-{end_line} of {total_lines})\n{'─' * 40}\n"
        )
        footer = ""
...
def list_files(
    root: str = ".", pattern: Optional[str] = None, limit: int = 100
) -> ToolResult:
    """
    ...
    try:
        ...
        if not all_items:
            return ToolResult(
                success=True,
                output=f"No files found in {root}"
    ...
    try:
        ...
        if test_path:
            if not is_path_safe(test_path):
                return ToolResult(
                    success=False, output="⛔ Path traversal not allowed."
                )
        ...
        return ToolResult(
            success=False, output=f"⏱️ Pytest timed out after {timeout} seconds."
        )
...
def run_enhanced_security_scans() -> ToolResult:
        ...
        if result.stdout.strip():
            try:
                ...
                for vuln in vulns[:10]:
                    ...
                    fix_str = (
                        f"→ {fix_versions[0]}" if fix_versions else "no fix available"
                    )
                    findings.append(
                        f"🔴 CRITICAL: {name}=={version} has vulnerability {vuln_id} ({fix_str})"
                    )
            ...
            if result.stdout.strip():
                try:
                    ...
                    for issue in data.get("results", [])[:15]:
                        ...
                        findings.append(
                            f"{icon} {severity}: {text[:80]} ({filename}:{line})"
                        )
                except json.JSONDecodeError:
                    pass
        except FileNotFoundError:
        ...
        if result.stdout.strip():
            try:
                ...
                for issue in issues[:10]:
                    ...
                    findings.append(
                        f"{icon} {category} [{code}]: {message[:60]} ({filename}:{row})"
                    )
            except json.JSONDecodeError:
                pass
    except FileNotFoundError:
    ...
    security_findings = [
        f for f in findings if any(icon in f for icon in ["🔴", "🟠", "🟡"])
    ]
...
AVAILABLE_TOOLS = {
    ...
    "get_latest_diff": {
        ...
        "parameters": {
            "change_id": {
                "type": "string",
                "description": "Optional commit hash to show specific changes",
                "required": False,
            }
        },
    },
    ...
    "search_repo": {
        ...
        "parameters": {
            ...
            "include_context": {
                "type": "boolean",
                "description": "Include surrounding lines (default true)",
                "required": False,
            },
        },
    },
    "open_file": {
        ...
        "parameters": {
            ...
            "end_line": {
                "type": "integer",
                "description": "Ending line number (default 50, max start+200)",
                "required": False,
            },
        },
    },
    ...
    "tail_logs": {
        ...
        "parameters": {
            "n_lines": {
                "type": "integer",
                "description": "Number of lines to show (max 100)",
                "required": False,
            }
        },
    },
...
def execute_tool(tool_name: str, arguments: Dict[str, Any]) -> ToolResult:
    ...
    try:
        ...
        return ToolResult(
            success=False, output=f"Invalid arguments for {tool_name}: {e}"
        )
    except Exception as e:
